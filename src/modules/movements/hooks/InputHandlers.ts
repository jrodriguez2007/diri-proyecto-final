...
export function useInputHandlers() {
  ...
  const [triggerGetInputById] = useLazyGetInputByIdQuery();
  const [createInput] = useCreateInputMutation();
  const [updateInput] = useUpdateInputMutation();
  const [deleteInput] = useDeleteInputMutation(); // recibe { idDocumento, idSerie, numero }
  const [updateStateInput] = useUpdateStateInputMutation();
  const [deleteMovementInput] = useDeleteMovementInputMutation();
  ...
  const entityHandler = new EntityHandlers<Logistic>(
    entityHooks,
    t('modules.form.create.title', {item: t('modules.inputs.label.title.singular')}), // Texto personalizable para creación.
    t('modules.form.edit.title', {item: t('modules.inputs.label.title.singular')}),  // Texto personalizable para edición.
  );
